...
# Define function for making data
make_data <- function(number){
  assign(paste0('data', number),
         get_or(variables[number]),
         envir = .GlobalEnv)
}
...
for (i in 1:length(variables)){
  ...
  result <- get(paste0('data', i))
  titles[i] <- paste0('\n## ', this_variable)
  table_list[[i]] <- DT::datatable(result)
  plot_list[[i]] <- make_plot(data = eval(parse(text = paste0('data', i))))
}
```
